"""再認証の試行記録（`reauth_attempts`）の掃除（Issue #240）。

## 何を消すか

作られてから **1 日**（`REAUTH_ATTEMPT_RETENTION_DAYS`）を過ぎた記録。境界は `<=`
（ちょうど 1 日たった行も消す。ほかの掃除ジョブと同じ）。

記録は、現パスワードによる再認証を伴う操作（秘密の質問・答えの変更、メールアドレスの
変更）を**受け付けるたび**に 1 行作られ、IP アドレスという個人情報を含む
（app/services/credentials.py）。放っておくと期限なく残り続ける。

## なぜ 1 日か

レート制限（`app/services/credentials.py` の `_count_recent`）が数えるのは**直近 15 分**の
...
使い方: `python -m app.jobs.cleanup_reauth_attempts`
"""
...
def cleanup_reauth_attempts(
    ...
    **`session` は新しく開いたものを渡すこと**（CLI の `main()` とテスト専用）。
    ...
    cutoff = current - timedelta(days=settings.REAUTH_ATTEMPT_RETENTION_DAYS)
...
def main() -> None:
    ...
    with Session(engine) as session:
        deleted = cleanup_reauth_attempts(session)
    logger.info("再認証の試行記録の掃除完了: %d 件を削除しました", deleted)
