...
jest.mock("../clientConfig", () => ({ getImageMaxDimension: jest.fn() }));

const mockPickImage = pickImage as jest.Mock;
const mockGetImageMaxDimension = getImageMaxDimension as jest.Mock;
...
beforeEach(() => {
  jest.clearAllMocks();
  mockGetImageMaxDimension.mockResolvedValue(2048);
});
...
describe("usePickAndSend", () => {
  it("選んだ画像を send に渡し、その結果を返す", async () => {
    ...
    const { result } = await renderHook(() => usePickAndSend<string>(send), { wrapper });
  ...
  it("Error 以外が投げられたら既定メッセージ", async () => {
    ...
    const { result } = await renderHook(() => usePickAndSend<string>(jest.fn()), { wrapper });
    await act(async () => {
      await result.current.pickAndSend();
    });
  ...
  it("取得したサーバー上限を画像選択へ渡す", async () => {
    mockGetImageMaxDimension.mockResolvedValue(1536);
    mockPickImage.mockResolvedValue(picked);
    const { result } = await renderHook(() => usePickAndSend<string>(jest.fn()), { wrapper });
