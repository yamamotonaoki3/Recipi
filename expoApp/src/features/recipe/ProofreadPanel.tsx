...
export function ProofreadPanel({
  ...
  return (
    ...
    >
      ...
      {Object.entries(
        suggestions.reduce<Record<string, ProofreadSuggestion[]>>((groups, suggestion) => {
          const section = sectionForId(suggestion.id);
          (groups[section] ??= []).push(suggestion);
          return groups;
        }, {}),
      ).map(([section, sectionSuggestions]) => (
        <View key={section} className="gap-2">
          <Text className="text-sm font-semibold text-neutral-800">{section}</Text>
