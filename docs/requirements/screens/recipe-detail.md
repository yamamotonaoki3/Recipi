...
- 入口: [ホーム](home.md)（フィード / 検索結果）/ [自分のレシピ一覧](my-recipes.md) / [ユーザープロフィール](user-profile.md) のカード、[通知一覧](notifications.md)の通知、[レシピ作成/編集](recipe-editor.md)の保存後、**他のレシピ詳細の材料リンク**（材料が自分の別レシピを参照している場合）。
...
8. 材料リスト: **材料グループごと**に表示。
   ...
   - **参照材料**（`refRecipe` あり）: `refRecipe.id` があれば材料名をリンク表示（下線 / アイコン）し、タップでそのレシピ詳細へ。`refRecipe.id` が null（参照先削除済み）なら、保持した材料名に「（削除済み）」を付けた通常テキストを表示し、リンクにはしない。
...
- 参照材料のリンクタップ → `refRecipe.id` がある場合だけ、そのレシピ詳細へ push。削除済み参照はリンクを表示しない。
...
- `GET /recipes/{id}`
- `POST /recipes/{id}/view`（閲覧記録・非同期・失敗は無視）
