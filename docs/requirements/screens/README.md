...
> **この配下の画面仕様は framework 非依存**。フロントは 2 トラック（TypeScript / Expo と Kotlin / Compose。[`../tech-stack.md`](../tech-stack.md)）あり、どちらも同じ画面仕様に従う。特定フレームワークの部品名ではなく、挙動・レイアウト・状態・遷移で記述する。
