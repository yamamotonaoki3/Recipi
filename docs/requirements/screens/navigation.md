...
- **モバイル（コンパクト幅）**: 画面下部に**ボトムナビゲーション**（4 destination）。
- **デスクトップ / タブレット（中〜広幅）**: 画面左に**ナビゲーションレール**（同じ 4 destination）。
...
- 「＋」はモーダル起動なので、閉じると直前の destination に戻る。4 項目での「＋」の配置（中央寄せできないため中央寄りの目立つ位置に置く）は実装時に確定（→ [`../todo.md`](../todo.md)）。
...
- **当面、アプリの全画面（スプラッシュ / ログイン / サインアップ / パスワードリセットを除く）はログイン必須**とする。4 destination とそこから開く画面（レシピ詳細を含む）すべてが対象。
...
| | モバイル | デスクトップ |
| --- | --- | --- |
| 主ナビ | 画面下部のボトムナビ（4 項目） | 画面左のナビゲーションレール（4 項目） |
| ＋（作成） | ボトムナビ内の目立つ位置（4 項目のため中央寄せはしない） | レール内。フルスクリーンではなく大きめのダイアログでも可（実装時に確定） |
