...
- **destination ごとに独立したナビゲーションスタック**を保持する。
- 別の destination から切り替えたときの表示（Issue #156）:
  - **全 destination（ホーム / 履歴 / 通知 / マイページ）** で、常にそのタブの**最初の画面**（レシピ一覧 / 履歴一覧 / 通知一覧 / マイページメニュー）を表示する（スタックは破棄する）。
  - ただしマイページで**プロフィール編集を未保存のまま**離れていた場合は、編集画面を表示して「変更を破棄しますか」の確認ダイアログを出す（破棄 → 最初の画面、キャンセル → 入力を残して編集画面のまま）。
...
- **ログイン / サインアップ成功後は常にホーム（ホーム destination）へ着地する**（Issue #53。「元々行こうとしていた画面へ戻す」挙動は採らない。ディープリンクで保護画面に弾かれた場合も、認証後はホームへ）。
...
| ＋（作成） | ボトムナビ中央（3 番目・目立つスタイル） | レール内。作成画面は中央の大きめのダイアログ（最大幅 768px・高さ 90%。背景クリック / Esc / × で閉じる。Issue #158 で確定。[recipe-editor.md](recipe-editor.md) §7） |
