...
- 検索バーに入力して確定すると、現在のタブ（フィード）を保ったまま検索結果画面へ（[search.md](search.md)）。お気に入りレシピタブでの検索併用の可否は実装時に確定（→ [../todo.md](../todo.md)）。
...
| `q` | 検索語（[search.md](search.md)）。`feed` と併用可（`favorites` との併用可否は → [../todo.md](../todo.md)） |
| `limit` / `cursor` | ページング |
...
- お気に入りレシピタブと検索（`q`）の併用可否 → [../todo.md](../todo.md)
