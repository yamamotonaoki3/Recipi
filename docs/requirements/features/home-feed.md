...
> **MVP でのスコープ（スコープ定義であって進捗記述ではない）**: Phase 4（MVP）のスコープとして定義されているのは **「全体」タブ ＋ ホーム上部の検索窓**のみ。「フォロー」「フォロワー」タブは Phase 5（フォロー機能）、「お気に入りレシピ」タブは Phase 6（お気に入り機能）のスコープ（[../roadmap.md](../roadmap.md)「MVP ライン」）。
>
...
>
> **frontend-ts 側の実装状況**: Phase 5・6 が完了済みのため、`HomeScreen.tsx` は現在 4 タブ（全体 / フォロー / フォロワー / お気に入りレシピ）すべてを有効化しており、いずれのタブも対応する backend API を実際に呼び出す。「準備中」プレースホルダ（Issue #42 時点の暫定実装）は Phase 5・6 の実装で解消済みで、現在は残っていない。
...
- [x] 「全体」に全ユーザーの公開レシピが新着順で出る（非公開は出ない）
- [x] 「フォロー」に、フォロー中ユーザーの公開レシピだけが出る
- [x] 「フォロワー」に、自分をフォローしているユーザーの公開レシピだけが出る
...
- [ ] 各タブはページングで全件たどれる
- [x] フォロー 0 / フォロワー 0 / お気に入り 0 のとき、それぞれ空状態メッセージが出る
