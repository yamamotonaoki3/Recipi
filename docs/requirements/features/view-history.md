...
- **保持上限**: 1 ユーザーあたり 200 件（`RECIPE_VIEWS_MAX_PER_USER`）。超えた分は定期ジョブ `python -m app.jobs.trim_recipe_views` が古い順に消す（閲覧のたびには削らない。Issue #72）。
...
- [x] レシピ詳細を開くと、そのレシピが閲覧履歴の先頭に出る
- [x] 同じレシピを見直すと、履歴の中で先頭に移動する（重複行は増えない）
- [x] 他人のレシピを見たあとそのレシピが非公開化されると、閲覧履歴一覧から消える
- [x] レシピが削除されると閲覧履歴一覧から消える
- [x] 「消去」で履歴が空になる
- [ ] 別端末で同じアカウントにログインすると同じ履歴が見える
- [x] 履歴一覧はカーソルページングで全件たどれる
- [x] `POST /recipes/{id}/view` が失敗してもレシピ詳細の表示に影響しない

## 8. 未確定・メモ

- ~~保持件数の上限と超過分の削除方式~~ → Issue #72 で確定（200 件・定期ジョブ）
