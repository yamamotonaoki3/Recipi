...
自分に関係する出来事（フォローされた・レシピがお気に入りされた・感想がついた・フォロー中ユーザーの新着レシピ）をアプリ内で知らせる。ヘッダーのベルマークに未読件数を出し、通知一覧画面で確認する。プッシュ通知は対象外。

## 2. 画面・UI

### トップアプリバーのベルマーク

- トップアプリバー（[../screens.md](../screens.md)）に**ベルアイコン**。未読があれば件数バッジ。
- タップで通知一覧画面へ。
...
- `recipe_favorited`: お気に入り成立時のみ。解除では作らない / 消さない（実装時に確定 → [../todo.md](../todo.md)）。
- `recipe_commented`: 感想の新規投稿時のみ。編集では作らない。
- `followee_new_recipe`: **公開**レシピの新規投稿時に、投稿者のフォロワー全員へ配布（fan-out）。非公開レシピ、後からの公開化では作らない（実装時に確定 → [../todo.md](../todo.md)）。
...
- お気に入り解除 / 非公開化に伴う通知の取り消し → 実装時に確定
- レシピの公開化（非公開 → 公開）で `followee_new_recipe` を出すか → 実装時に確定
- プッシュ通知・メール通知は対象外（将来）
