...
- **他人のレシピが非公開になったら、そのレシピの通知は一覧にも未読数にも出さない**（行は消さず、公開に戻れば再び出る。ただし既読から 90 日の保持期間を過ぎた既読通知は、隠れている間でも定期ジョブが消すので、再公開しても戻らない。Issue #72 で決定）。自分のレシピの通知は非公開でも出る（Issue #70 で確定）。
...
- [x] フォローされると被フォロー者に `followed` が 1 件作られる
- [x] 自分のレシピが他人にお気に入りされると `recipe_favorited` が作られる
- [x] お気に入りを解除しても、それ以前に作られた `recipe_favorited` 通知は残る
- [x] 自分のレシピに他人が感想を書くと `recipe_commented` が作られる
- [x] フォロー中ユーザーが公開レシピを投稿すると、（通常運用時）その時点のフォロワー全員に `followee_new_recipe` が作られる。障害回収が遅れた場合は配布処理の実行時点のフォロワーが対象になる（[../processing-model.md](../processing-model.md) §7・§9）
- [x] 自分の操作で自分あての通知は作られない
- [x] 未読バッジの数が `unread-count` と一致する
- [x] 通知をタップ / 「すべて既読」で `read_at` が入り、バッジが減る
- [x] 対象レシピ / 感想が削除された通知は一覧に出ない
