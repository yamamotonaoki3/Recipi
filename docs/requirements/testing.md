...
| ---- | ---------------- | ---------------- | ---------------- |
| 測定日 / マシン | 2026-09-17 / Windows 11（開発機）。uvicorn 単一プロセス ＋ ローカル Docker（PostgreSQL 18・MinIO）。k6 v2.1.0 | 同左 | 未測定 |
| データ量 | ユーザー 200・レシピ 3,000（`seed_perf` の既定） | 同左 | 同左 |
| feed p50 / p95 / p99 | 15.94ms / **48.70ms** / 67.65ms | 76.05ms / **158.08ms** / 195.78ms（全区間。下記の注記参照） | 未測定（急増中 / 回復区間を分けて書く） |
| search p50 / p95 / p99 | 19.12ms / **48.07ms** / 66.78ms | 79.40ms / **161.19ms** / 199.15ms（同上） | 未測定（同上） |
| detail p50 / p95 / p99 | 13.49ms / 48.27ms / 68.87ms | 72.95ms / 166.02ms / 202.67ms | 未測定 |
| RPS（1 秒あたりのリクエスト数） | 58.34 req/s（7 分で 24,595 リクエスト・4,916 反復） | 119.07 req/s（16 分で 114,575 リクエスト・22,897 反復） | 未測定 |
| 失敗率 / 5xx 件数 | 0.00%（0 / 24,595）。check は 100.00%（29,511 / 29,511） | 0.00%（0 / 114,575）・**5xx 0 件**。check は 100.00%（137,472 / 137,472） | 未測定（エラーの内訳も書く） |
| 最初に詰まった箇所 | — | **詰まりは観測されなかった**（5xx 0 件・失敗 0 件・中断された反復 0 件）。最大値のみ 639ms まで伸びた | — |
| 回復までの時間 | — | — | 未測定 |
| 判定 | **合格**（閾値 4 つすべて。feed / search の p95 < 300ms） | **合格**（閾値 5 つすべて。p95 < 600ms・失敗率 < 1%・5xx 0 件） | 未測定 |
