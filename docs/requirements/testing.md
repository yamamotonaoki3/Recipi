...
ストレス・スパイクは Issue #147・#148 で追加する。
...
| 項目 | 平均負荷（#146） |
| ---- | ---------------- |
| 測定日 / マシン | 2026-09-17 / Windows 11（開発機）。uvicorn 単一プロセス ＋ ローカル Docker（PostgreSQL 18・MinIO）。k6 v2.1.0 |
| データ量 | ユーザー 200・レシピ 3,000（`seed_perf` の既定） |
| feed p50 / p95 / p99 | 15.94ms / **48.70ms** / 67.65ms |
| search p50 / p95 / p99 | 19.12ms / **48.07ms** / 66.78ms |
| detail p50 / p95 / p99 | 13.49ms / 48.27ms / 68.87ms |
| RPS（1 秒あたりのリクエスト数） | 58.34 req/s（7 分で 24,595 リクエスト・4,916 反復） |
| 失敗率 | 0.00%（0 / 24,595）。check は 100.00%（29,511 / 29,511） |
| 判定 | **合格**（閾値 4 つすべて。feed / search の p95 < 300ms） |
