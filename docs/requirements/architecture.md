...
クライアント ─HTTPS─> API Gateway HTTP API ─ VPC Link（private サブネット・専用 SG）
                        └─> 内部ALB(private) ─> ECS Fargate(private):8000
...
VPC 2AZ: public x2（IGW・NAT 1 つ）/ private x2（ALB・ECS・RDS Multi-AZ・VPC Link の ENI）
         S3 へはゲートウェイ型エンドポイント（無料。ECR のイメージ取得が NAT を通らない）
```

- **通信の許可は必要な経路だけ**に絞る（`security-groups.tf`）: 「VPC Link の SG → ALB の 8000 番」「ALB の SG → ECS の 8000 番」「ECS の SG → RDS の 5432 番」。ALB・ECSのタスクに公開 IP は付けず、RDS も外部に公開しない。外向きの通信は NAT 経由。
- **DB** は RDS PostgreSQL 18（db.t4g.micro・Multi-AZ・private サブネット）。プライマリ障害時は別 AZ の待機系へ AWS が自動フェイルオーバーし、アプリは同じ RDS エンドポイントを使い続ける。**画像**は非公開の S3 バケット。**アバター**は CloudFront の OAC 経由で配信し（`uploads/*` の `GetObject` だけ許可）、**レシピのサムネ・手順画像・感想画像**は `private/` に置いて**期限付きの署名付き URL**で配信する（CloudFront 経由でも 403。Issue #185）。アプリは ECS のタスクロールで S3 を読み書きし、アクセスキーを持たない（Issue #166）。
...
- **内部ALBを使う**。API Gateway HTTP API ＋ VPC Linkから内部ALBへ接続し、ALBのヘルスチェックとターゲット振り分けで複数ECSタスクへ分散する。学習用のため、検証しない期間はAWSリソースをdestroyして固定費を止める。
- **NAT Gateway は 1 つ**（1 つ目の AZ）。その AZ が落ちると ECS が外に出られない。冗長化するなら AZ ごとに置く。
- **RDS は Multi-AZ・`skip_final_snapshot = true`・`deletion_protection = false`**。Multi-AZ は可用性向上用であり、バックアップではない。destroy でデータは消える。長く使う本番なら最終スナップショットと削除保護を有効にする。
