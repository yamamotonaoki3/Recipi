...
**なぜ使わないか**: MVP の規模（フォロワー数百〜数千）では `BackgroundTasks` で足りる見込みで、Redis を足すコストのほうが大きい。探索的性能測定（フォロワー1,000人まで問題なし）を踏まえ、学習用アプリのため導入しないことを確定した（§3・[todo.md](todo.md) #18・#50）。
...
| 即時の後処理の実行基盤（MVP） | FastAPI **`BackgroundTasks`**（プロセス内・ブローカー無し） | 追加インフラ（Redis / ワーカー）が要らず Phase 0 の scaffold が軽い。「従来方式を先に学ぶ」方針（[../../CLAUDE.md](../../CLAUDE.md) 学習方針・[todo.md](todo.md) #39）に沿う。追加のマネージドサービスなしで、本番（AWS の ECS Fargate・[architecture.md](architecture.md) §本番デプロイ）でもそのまま動く。MVP 規模なら「まれな取りこぼし」のコスト < 「キュー基盤の運用の複雑さ」 |
...
| 専用ジョブキュー（arq / Celery + Redis） | **導入しない（確定・2026-09-22）** | 学習用アプリで実運用の負荷が想定されないため、Issue #248 の探索的測定（フォロワー 1,000 人まで問題なし）を踏まえて `BackgroundTasks` ＋ 定期スイープのまま確定する。導入コストが機能価値に見合わない（[todo.md](todo.md) #18・#50） |
...
| パスワード再設定の試行記録の掃除（`python -m app.jobs.cleanup_password_reset_attempts`） | 作られてから 1 日（`PASSWORD_RESET_ATTEMPT_RETENTION_DAYS`）を過ぎた `password_reset_attempts` を削除（境界は `<=`。成功した request の記録も含む。users に外部キーが無くアカウント削除では消えないため）。`BATCH_SIZE` 行ずつ・`SKIP LOCKED`・`created_at` のインデックスで探す（Issue #85） | 対象を都度判定 | 1 日（Issue #85 で確定） | 次回実行で回収 | レート制限の正しさは直近 15 分の件数の判定で担保済み。物理削除は個人情報（メール・IP）を必要以上に持たないためで、急がない |
...
### fan-out の移行検討開始条件（参考・Issue #260。導入しない確定のため現在は不使用）

専用ジョブキューへの移行は行わないと確定した（上記参照）。以下は将来、実運用の負荷が生じて再検討する場合の参考として残す判断基準で、現状は適用しない。

- 通常配布: 固定到着率で投入量未達（`dropped_iterations > 0`）または、投稿停止後60秒以内に
  99%以上の outbox が処理済みにならない。
- 障害回収: スイープ1回あたり最大500行を前提に、未処理行が次回起動までに減らず、
  30分以上未処理が残る、または流入件数が1回の上限500行を継続的に超える。
- 通知の正しさ: 通知件数または受信者集合の不一致が1件でも発生した場合は、キュー導入ではなく
  先に配布処理・再試行・DB制約を調査する。

上記は通常配布（BackgroundTasks）と障害回収（10分間隔スイープ）を混同しないための基準である。
API応答時間だけが遅い場合やDB書き込みがボトルネックの場合は、ジョブキューを追加しても解決しないため、
実行基盤・接続プール・SQLを先に調査する。
...
- ~~本番のジョブ実行基盤（cron / コンテナスケジューラ）の具体と、多重起動防止の方法~~ → **Issue #173 で確定**: **EventBridge Scheduler → ECS RunTask**（API と同じタスク定義の `command` を上書きして 1 回だけ動かす。常駐ワーカーなし）。**多重起動の防止はスケジューラ側では行わない**（各ジョブが `FOR UPDATE SKIP LOCKED` や advisory lock で、同時に動いても壊れないため）。失敗は 4 経路（異常終了・起動失敗・スケジューラの起動失敗・終了コード 0 のままの失敗をログの件数で）で検知し、SNS のメールに通知する。詳細は `infra/terraform/README.md`
- ~~各定期バッチの実行頻度~~ → **Issue #173 で確定**（§8 の台帳と `infra/terraform/scheduler.tf`）: 通知スイープ 10 分・ストレージ削除 15 分・一時アップロード GC 1 時間・その他の掃除と数え直しは日次（日本時間 2:10〜2:50 に 10 分ずつずらす）
...
- ~~各ジョブを何分おきに動かすか・多重起動防止・監視~~ → **Issue #173 で確定**（上と同じ）
- ~~大量フォロワー時の fan-out 性能測定と、専用ジョブキュー導入の判断（Phase 10 以降）~~ → 確定（2026-09-22）: 導入しない（[todo.md](todo.md) #18・#50）
