...
| 専用ジョブキュー（arq / Celery + Redis） | **Phase 10 以降に再検討** | Issue #248 の探索的測定で桁を把握し、厳密な測定と移行開始条件は Issue #260 で決める。先に入れると学習・運用コストが機能価値より先行する |
...
### fan-out の移行検討開始条件（Issue #260）

専用ジョブキューの導入は、次の利用者影響が**同じ測定条件で2回連続**して確認された場合に
検討を開始する。測定値に合わせて合格基準を下げるのではなく、通知の用途上必要な時間を基準にする。
