...
| 53 | **Android E2E（Appium + WebdriverIO）の要素検索コマンドが機能しない不具合の再調査**（2026-09-06）: Web 版（Playwright）は安定動作を確認済みだが、Android 版は画面遷移後に `findElement` / `isExisting` 等の要素検索コマンド全般が失敗し続ける（`getPageSource` だけは正常で、実際の画面は正しく描画されている）という深い不具合が判明。待機時間の延長・`isExisting` への切り替え等、複数の対策を試しても解消しなかったため、`.github/workflows/e2e.yml` の `android` ジョブは `continue-on-error: true` にして暫定的に非ブロッキング化した。詳細は [lessons-learned.md](../lessons-learned.md) 参照。再調査の候補: Appium サーバーのバージョンを変える、セッションを都度作り直す、`getPageSource` を自前でパースして要素を探す等 | 未定（手動確認で代替中） | [testing.md](testing.md), [lessons-learned.md](../lessons-learned.md) |
