...
- サムネイル画像（`recipes.thumbnail_key` から生成した表示用 URL。無ければプレースホルダ）
- タイトル（1〜2 行で省略）
- 投稿者（アバター + 表示名）
...
1. メールアドレス入力 → 送信
2. 登録済みの質問文を表示
3. 同じ画面で答え + 新しいパスワード + 確認（表示トグル・一致チェック）をまとめて入力 → 1 回で送信
4. `POST /auth/password-reset/confirm` の成功後、ログイン画面へ
- 新しいパスワードと確認が不一致の場合はクライアントでエラー表示し、API 呼び出しをブロック
