...
| Phase 4 | ボトムナビゲーション / ナビゲーションレール（4 destination）＋ ホーム「全体」タブ ＋ ホーム上部の常時固定検索窓（`q`、スペース分割・正規化、表示中サブタブ内で絞り込み）＋ **閲覧履歴**（`recipe_views`、`POST /recipes/{id}/view`、`GET`/`DELETE /users/me/history`、閲覧履歴画面。レシピ詳細=Phase 2 とナビ=Phase 4 に依存） | [screens/navigation.md](screens/navigation.md), [screens/home.md](screens/home.md), [screens/history.md](screens/history.md), [features/home-feed.md](features/home-feed.md), [features/search.md](features/search.md), [features/view-history.md](features/view-history.md) |
