...
| 閲覧履歴 | 最近見たレシピの一覧。レシピ詳細を開くとサーバーに記録され（`recipe_views`、レシピごとに 1 行・最後に見た時刻で並ぶ）、同じアカウントなら端末をまたいで見える。ボトムナビ / レールの「履歴」destination。[features/view-history.md](features/view-history.md) 参照 |
...
| ボトムナビゲーション | モバイルで画面下部に置く主ナビ。5 destination（ホーム / 履歴 / ＋ / 通知 / マイページ。「＋」は中央）。[screens/navigation.md](screens/navigation.md) 参照 |
...
| destination | ボトムナビ / レールで切り替える主要な行き先。5 つ（ホーム / 履歴 / ＋ / 通知 / マイページ） |
...
| AI 誤字脱字チェック | レシピ作成 / 編集画面でタイトル・説明・手順・材料名・材料グループ名の誤字脱字を AI がチェックし修正案を出す機能（Phase 11・MVP 対象外）。自動適用しない。バックエンドの校正サービスは `AI_PROVIDER` で切替（dev = Ollamaローカル推論 / prod = Anthropic API / test = スタブ）。[features/ai-proofread.md](features/ai-proofread.md) 参照 |
