...
20. **「既存テストが通っている」は、その画面全体が触れることの保証ではない**。Android E2E 3 本はどれも `step-0-body` より下へスクロールしたことが無く、フォーム末尾は誰も触っていなかった。`recipe-crud.e2e.ts` には以前から「ここで `scrollToId` を使うと UiScrollable が見つけられず失敗する」というコメントがあり、同じ現象を回避したまま残っていた。**回避策のコメントは、未解決の不具合の目印として読む**。
