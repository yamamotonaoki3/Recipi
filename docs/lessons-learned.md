...
11. **Appium + WebdriverIO の Android でも、上記10とは別系統の深い不具合に遭遇した**（Playwright への置き換え後）。症状: 画面遷移（例: 「新規登録」タップ後のサインアップ画面表示）の後、`waitForDisplayed()` はもちろん `isExisting()` まで含めた `findElement` 系のコマンド全般が、対象要素に対して待機時間いっぱい（30秒・60秒と伸ばしても）ずっと失敗し続けた。しかし失敗直後に `getPageSource()` で取得した画面構造（XML）を見ると、当該要素は `resource-id` も正しく、`displayed="true"` の状態で確かに存在していた（スクリーンショットでも画面は正しく描画されていることを確認）。つまり「アプリは正常」「`getPageSource` は正常」「`findElement` 系コマンドだけが機能しない」という状態。原因の切り分けとして試した対策（`waitForDisplayed` → `isExisting` を `browser.waitUntil()` で自前ポーリングに変更、要素を毎回 `$()` で取り直す＝stale element 対策も反映済み）でも解消しなかった。**対応**: これ以上の深追いはせず、`.github/workflows/e2e.yml` の `android` ジョブに `continue-on-error: true` を付けて暫定的に非ブロッキング化し、Web 版（Playwright、安定動作確認済み）の足を引っ張らないようにした（[todo.md](requirements/todo.md) #53 に再調査事項として記録）。**教訓**: `getPageSource`（静的なスナップショット取得）と `findElement`（対話的なコマンド）が同じ WebDriver セッション内で乖離するのは、ドライバ・セッション管理側の深い不具合の兆候であり、テストコード側の待機戦略をいくら変えても直らない可能性が高い。この種の兆候が出たら、コード側の工夫を重ねるより先に「暫定的に非ブロッキング化して手動確認で補う」という損切りの判断を検討する。
